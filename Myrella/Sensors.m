...
-(float) calcXValue:(NSData *)data {
    ...
    self.lastX = ((scratchVal[0] * 1.0) / (256 / IMU3000_RANGE)) * -1;
...
-(float) calcYValue:(NSData *)data {
    ...
    char scratchVal[data.length];
    [data getBytes:&scratchVal length:3];
    self.lastX = ((scratchVal[1] * 1.0) / (256 / IMU3000_RANGE)) * -1;
    return (self.lastX - self.calX);
}
-(float) calcZValue:(NSData *)data {
    char scratchVal[data.length];
    [data getBytes:&scratchVal length:3];
    self.lastX = ((scratchVal[2] * 1.0) / (256 / IMU3000_RANGE));
    return (self.lastX - self.calX);
}
+(float) getRange {
...
+(float) calcXValue:(NSData *)data {
    ...
    return (((float)rawX * 1.0) / ( 65536 / KXTJ9_RANGE ));
}
+(float) calcYValue:(NSData *)data {
    //Orientation of sensor on board means we need to swap Y (multiplying with -1)
    char scratchVal[6];
    ...
    return (((float)rawX * 1.0) / ( 65536 / KXTJ9_RANGE )) * -1;
}
+(float) calcZValue:(NSData *)data {
    char scratchVal[6];
    [data getBytes:&scratchVal length:6];
    int16_t rawX = (scratchVal[4] & 0xff) | ((scratchVal[5] << 8) & 0xff00);
    return (((float)rawX * 1.0) / ( 65536 / KXTJ9_RANGE ));
}
+(float) getRange {
    return KXTJ9_RANGE;
